...
class GP:
    ...
    def run_gp(self, pop_size, epochs):
        ...
        for i in range(epochs):
            ...
            while len(new_pop) < len(population):
                ...
                parent_one = self.parent_select(population)
                parent_two = self.parent_select(population)
    ...
    def parent_select(self, population, parent_size=6):
        """
        ...
        parent_size -- number of individuals randomly sampled to participate

        Returns:
        winner -- individual with the best fitness out of the parent_size participants
        """

        # randomly sample participants
        indices = np.random.choice(len(population), parent_size)
